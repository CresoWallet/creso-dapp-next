...
const NavItem = ({ pathName, href, icon, hoverIcon, label, onClick }) => {
  ...
  return (
    <Link href={href} className="group relative">
      <div
        className={`w-8 h-10 group-hover:bg-white absolute top-5 ${pathName === href ? "bg-white" : ""
          } `}
      ></div>
      <div
      ...
      ></div>
      <div
        className={`flex flex-row gap-4 items-center group-hover:bg-white rounded-r-full !py-4 navActiveMenu  ${pathName === href ? "bg-white" : ""
          }`}
        onClick={onClick}
      ...
      >
        ...
        <p
          className={`text-[#B1A6F8] text-sm group-hover:text-black font-medium hidden lg:block ${pathName === href ? "text-black" : ""
            }`}
        >
          {label}
        </p>
      ...
      <div
        className={`w-8 h-10 group-hover:bg-white absolute bottom-5 ${pathName === href ? "bg-white" : ""
          }`}
      ></div>
      <div
        className={`w-10 h-9 bg-[#2100EC] relative rounded-l-full left-[3px] lg:left-0 ${pathName === href ? "bg-[#2100EC]" : ""
